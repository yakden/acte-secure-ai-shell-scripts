...
**Anthropic model-specific and feature caveats.** Anthropic's newest "Covered Models" require 30-day data retention and are not available under ZDR; requests from non-compliant orgs return a 400 error [65][70]. Stateful features fall outside ZDR — Batch API (~29-day retention), Files API (retained until deleted), code execution (up to 30 days), Agent Skills, MCP connector, and Managed Agents [65][72].
...
Anthropic's Responsible Disclosure Policy asks researchers to report in good faith, coordinate disclosure timing, and not publicly disclose vulnerability details until Anthropic gives written confirmation; the infrastructure bug bounty excludes model-content issues (jailbreaks, harmful content, hallucinations), which route to the separate Model Safety Bug Bounty (public since May 7, 2026, participation under NDA) [136][139][140][141]. Independent evaluation underscores why dual-use safeguards tightened: the UK AISI reported an Anthropic frontier model preview achieving ~73% on expert-level cyber tasks no model could complete before April 2025, and Cybench results rose from 35.9% (Sonnet 3.7) to 76.5% (Sonnet 4.5) at 10 attempts [145][146][147].
